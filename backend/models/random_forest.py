...
def run_random_forest(file_stream, target_column):
    ...
    y_proba = rf.predict_proba(X_test)[:, 1] if len(np.unique(y)) == 2 else None
    ...
    model_registry["random_forest"]["model"] = rf
    model_registry["random_forest"]["scaler"] = scaler
    model_registry["random_forest"]["features"] = list(X.columns)
    model_registry["random_forest"]["encoders"] = encoders
    model_registry["random_forest"]["target_encoder"] = target_encoder
