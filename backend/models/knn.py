...
def run_knn(file_stream, target_column, n_neighbors=5):
    ...
    y_proba = knn.predict_proba(X_test)[:, 1] if len(np.unique(y)) == 2 else None
    ...
    model_registry["knn"]["model"] = knn
    model_registry["knn"]["scaler"] = scaler
    model_registry["knn"]["features"] = list(X.columns)
    model_registry["knn"]["encoders"] = encoders
    model_registry["knn"]["target_encoder"] = target_encoder

    knn_features = list(X.columns)
