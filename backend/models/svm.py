...
def run_svm(file_stream, target_column):
    ...
    y_proba = svm.predict_proba(X_test)[:, 1] if len(np.unique(y)) == 2 else None
    ...
    model_registry["svm"]["model"] = svm
    model_registry["svm"]["scaler"] = scaler
    model_registry["svm"]["features"] = list(X.columns)
    model_registry["svm"]["encoders"] = encoders
    model_registry["svm"]["target_encoder"] = target_encoder
    svm_features = list(X.columns)
