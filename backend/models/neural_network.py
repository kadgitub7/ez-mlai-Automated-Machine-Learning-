...
def run_neural_network(file_stream, target_column):
    ...
    y_proba = mlp.predict_proba(X_test)[:, 1] if len(np.unique(y)) == 2 else None
    ...
    model_registry["neural_network"]["model"] = mlp
    model_registry["neural_network"]["scaler"] = scaler
    model_registry["neural_network"]["features"] = list(X.columns)
    model_registry["neural_network"]["encoders"] = encoders
    model_registry["neural_network"]["target_encoder"] = target_encoder
    nn_features = list(X.columns)
